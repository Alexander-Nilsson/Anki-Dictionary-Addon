...
class Forvo(QRunnable):
    ...
    def decodeURL(self, onclick_data):
        ...
        try:
            ...
            if len(parts) >= 3:
                ...
                decoded_link = base64.b64decode(base64_audio.encode("ascii")).decode(
                    "ascii"
                )
    ...
    def generateURLS(self, content):
        ...
        for noise in lang_container.select(
            f"ul#phrase-pronunciations-list-{self.langShortCut}-"
        ):
            noise.decompose()
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.signals.noResults.emit(
                    f"Forvo returned status code {response.status_code}"
                )
        except Exception as e:
    ...
    def attemptFetchForvoLinks(self, term):
        ...
        self.signals.noResults.emit(
            f'No pronunciations found for "{term}" in {self.selLang}'
        )
        return False
