    ...
    for lang in index.get("languages", []):
        ...
        for to_lang in lang.get("to_languages", []):
            ...
            for d in to_lang.get("dictionaries", []):
                ...
                if not url.startswith("http"):
                    url = SERVER_ROOT + url
                dicts.append({"lang": lang_name, "name": f"{d['name']} ({to_name})", "url": url})
    return dicts
        ...
        for i, d in enumerate(dictionaries):
            ...
            try:
                ...
                if resp.status_code != 200:
                    results.append({"name": name, "status": "FAIL", "error": f"HTTP {resp.status_code}"})
                    continue
                ...
                if count > 0:
                    ...
                    results.append({"name": name, "status": "FAIL", "error": "No entries imported"})
